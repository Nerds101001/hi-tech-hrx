            ...
            <div class="review-card p-6 mb-6">
                <div class="d-flex align-items-center gap-3 mb-6">
                    ...
                    <div>
                        ...
                        <span class="badge bg-label-info">{{ $user->roles->first()->name ?? 'N/A' }}</span>
                    </div>
                </div>
                ...
                <div class="row g-4">
                    ...
                    @forelse($files as $file)
                        <div class="col-md-6">
                            <a href="{{ asset('storage/'.$file) }}" target="_blank" class="document-preview-card">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header">
                <h5 class="modal-title fw-bold">Request Resubmission</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('employees.onboarding.resubmit', $user->id) }}" method="POST">
                @csrf
                <div class="modal-body">
                    <label class="form-label fw-bold">What needs to be corrected? <span class="text-danger">*</span></label>
                    <textarea name="notes" class="form-control" rows="4" placeholder="e.g. Please upload a clearer copy of your PAN card and verify the bank account number." required></textarea>
                    <p class="small text-muted mt-2">
                        This note will be sent to the user via email and their onboarding form will be unlocked for editing.
                    </p>
                </div>
                <div class="modal-footer border-0">
                    <button type="button" class="btn btn-label-secondary" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" class="btn btn-danger px-6">Send Request</button>
                </div>
            </form>
        </div>
    </div>
