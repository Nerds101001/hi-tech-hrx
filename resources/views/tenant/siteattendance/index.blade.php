...
@section('title', __('Site Management'))
...
@section('page-script')
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      const dt_table = $('.datatables-sites');
      if (dt_table.length) {
        dt_table.DataTable({
          processing: true,
          serverSide: true,
          ajax: "{{ route('siteattendance.indexAjax') }}",
          columns: [
            { data: 'id' },
            { data: 'name' },
            { data: 'address' },
          ...
          buttons: [
            {
              text: '<i class="bx bx-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">Add New Site</span>',
...
<div class="row g-6 px-4">
  <div class="col-lg-12">
    <x-hero-banner 
      ...
      subtitle="Defines physical sites and branches for attendance"
      icon="bx-map-pin"
      gradient="primary"
    />
  </div>
  ...
  <div class="col-12 mt-6">
    <div class="hitech-card animate__animated animate__fadeInUp">
      <div class="hitech-card-header border-bottom">
        <h5 class="title mb-0">Sites List</h5>
      </div>
      <div class="card-datatable table-responsive p-0">
        <table class="datatables-sites table table-hover border-top mb-0">
          <thead>
            <tr>
              <th>ID</th>
              <th>Site Name</th>
