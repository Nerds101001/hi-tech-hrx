...
<div class="layout-full-width animate__animated animate__fadeIn">
  ...
  <div class="d-flex justify-content-between align-items-center mb-6 px-4">
    <h3 class="mb-0 fw-bold text-heading" style="font-size: 1.5rem;">Job Categories</h3>
    @can('Create Job Category')
      <a href="#" data-url="{{ route('job-category.create') }}" data-ajax-popup="true" data-size="md" data-title="Create New Job Category" class="btn btn-hitech-primary shadow-sm">
        ...
        <div class="row align-items-center g-4">
          <div class="col-md-9 d-flex align-items-center gap-3 w-100">
            <div class="search-wrapper-hitech flex-grow-1">
              ...
              <input type="text" class="form-control border-0 bg-transparent shadow-none" placeholder="Search Categories..." id="customSearchInput">
            </div>
        ...
        <table class="datatable table m-0 shadow-none table-hover">
            <thead>
              <tr class="text-muted small text-uppercase">
                <th class="border-bottom">Category Title</th>
            ...
            <tbody>
              @foreach ($categories as $category)
                <tr>
                  <td><span class="fw-bold text-heading">{{ $category->title }}</span></td>
                  <td class="text-end">
                    <div class="d-flex justify-content-end gap-2">
                      @can('Edit Job Category')
                        <a href="#" data-url="{{ route('job-category.edit', $category->id) }}" data-ajax-popup="true" data-title="Edit Job Category" class="btn btn-icon btn-label-info btn-sm" data-bs-toggle="tooltip" title="Edit">
                          <i class="bx bx-edit-alt"></i>
                        </a>
                      @endcan

                      @can('Delete Job Category')
                        <form action="{{ route('job-category.destroy', $category->id) }}" method="POST" class="d-inline">
                          @csrf
                          @method('DELETE')
