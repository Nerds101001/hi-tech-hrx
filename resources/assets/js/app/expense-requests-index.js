      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            if (full['document_url_formatted']) {
              return '<a href="' + full['document_url_formatted'] + '" class="glightbox"><img src="' + full['document_url_formatted'] + '" alt="Proof" height="40" class="rounded border shadow-sm" /></a>';
            }
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            actionsHtml += `<button class="btn btn-sm btn-icon expense-request-details hitech-action-icon" data-id="${full['id']}" data-bs-toggle="modal" data-bs-target="#modalExpenseRequestDetails" title="View Details"><i class="bx bx-show"></i></button>`;

            if (full['status'] === 'pending') {
              actionsHtml += `<button class="btn btn-sm btn-icon hitech-action-icon text-success quick-expense-approve" data-id="${full['id']}" title="Quick Approve"><i class="bx bx-check-circle"></i></button>`;
              actionsHtml += `<button class="btn btn-sm btn-icon hitech-action-icon text-danger quick-expense-reject" data-id="${full['id']}" title="Quick Reject"><i class="bx bx-x-circle"></i></button>`;
            }
